<template>
    ...
    <div v-if="dataLoaded">
      <div class="mt-8 dark:text-white">
        ...
        <div class="flex justify-center mt-4 mb-4 space-x-4">
          <button @click="filterOffers('all')" :class="{ 'p-2 rounded bg-blue-500 text-white': currentFilter === 'all' }">All</button>
          <button @click="filterOffers('accepted')" :class="{ 'p-2 rounded  bg-green-500 text-white': currentFilter === 'accepted' }">Accepted</button>
          <button @click="filterOffers('rejected')" :class="{ 'p-2 rounded bg-red-500 text-white': currentFilter === 'rejected' }">Rejected</button>
          <button @click="filterOffers('pending')" :class="{ 'p-2 rounded  bg-yellow-500 text-white': currentFilter === 'pending' }">Pending</button>
        </div>
  ...
  const getTutorSentOffers = async () => {
    const response = await fetch(`http://localhost:8000/api/tutor/${store.user.tutor_id}/offers`, {
      method: 'GET',
      headers: {
